...
1. Run ```subscription-manager list --organization-id 1``` and attach the appropriate subscription to ```rhel-7-server-ak``` like so: ```activation-key add-subscription --organization-id 1 --name "rhel-7-server-ak"  --quantity 1 --subscription-id [SUBSCRIPTION ID HERE]```.
...
2. Run ```subscription-manager list --organization-id 1``` and attach the appropriate subscription to ```oscp-server-ak``` like so: ```activation-key add-subscription --organization-id 1 --name "oscp-server-ak"  --quantity 1 --subscription-id [SUBSCRIPTION ID HERE]```.
